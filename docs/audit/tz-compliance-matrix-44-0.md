...
132 предсуществующих запрещённых термина (`backend`, `manifest`, `API`, `PoP`, `batch`, `sidecar`, `Chromium`, `daemon`) присутствуют в существующих шаблонах и бизнес-логике. Задокументированы как `UI_AUDIT_001` в реестре отклонений. Не исправлены — требуют отдельного этапа рефакторинга существующей бизнес-логики.
