...
**Задокументировано, не исправлено.** Данные термины являются частью предсуществующей бизнес-логики и их замена требует отдельного этапа рефакторинга:

- Затрагивает существующие имена модулей, функций и переменных
- Может нарушить обратную совместимость API
- Требует координации с командами КСО и тестирования

### Mitigation

- Видимый UI (production) очищен: grep — 0 совпадений по запрещённым терминам
- Внутренние термины задокументированы для будущего рефакторинга
- План: отдельный этап рефакторинга после закрытия физических P0 gates

### Target

Отдельный этап рефакторинга (после RC0, до v1 production).
