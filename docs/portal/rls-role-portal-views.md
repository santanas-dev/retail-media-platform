...
| Сервис КСО | `device_service` | Machine-only, только deployment |
...
### Сервис КСО (device_service)
- **Основной экран:** Deployment
- **Доступ:** Только развёртывание
- **RLS:** store_scope, device_scope
- **Действия:** Управление КСО, deployment
- **Запрещено:** Всё, кроме deployment — нет human UI-доступа к порталу
- **MFA:** Не требуется
- **Аудит:** Не требуется (machine-only)
- **Назначение:** Машинная учётная запись для автоматизированного обслуживания КСО
...
- `apps/portal-web/security_contract.py` — `RolePortalView`, `PAGE_ROLE_MATRIX`, `RLS_RULES`, `FORBIDDEN_FIELDS_ALL`, `NETWORK_HIERARCHY`, `COMMERCIAL_HIERARCHY`
- `apps/portal-web/templates/pages/admin.html` — Role-Based Portal Views + RLS Matrix UI
