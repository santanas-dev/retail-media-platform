...
**Документ:** `docs/kso/kso-runtime-release-package-contract.md`
**Builder:** `infra/kso-linux/release/kso_release_package_builder.py`

Структура релизного пакета `kso-runtime-<version>.tar.gz` для внутреннего ИТ-контура:
VERSION + MANIFEST.json + CHECKSUMS.sha256 + apps + player_shell + infra + docs.

**Что внутри:** builder dry-run/build, versioning (MAJOR.MINOR.PATCH),
checksums, install layout (releases/<version>/ + current symlink),
rollback contract.

**Запрещено:** публичный GitHub/CDN, автообновление из интернета,
runtime data в пакете, удаление PoP/cache при rollback.
