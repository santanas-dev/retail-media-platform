"""Tests for kso_player.cli simulate-step — simulation CLI, no media played."""
...
def _setup_ready(root):
    ...
    (mc / "item.png").write_bytes(PNG)


# ══════════════════════════════════════════════════════════════════════

class TestSimulateStepHelp(unittest.TestCase):
    def test_help(self):
        out, err, rc = _run_cli("--help")
        self.assertEqual(rc, 0)
        self.assertIn("simulate-step", out)

    def test_simulate_step_help(self):
        out, err, rc = _run_cli("simulate-step", "--help")
        self.assertEqual(rc, 0)
        self.assertIn("--root", out)
        self.assertIn("--state", out)


class TestSimulateStepHappy(unittest.TestCase):
    def setUp(self):
        self.root = Path(tempfile.mkdtemp())
        _setup_ready(self.root)

    def test_idle_would_play(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "idle")
        self.assertEqual(rc, 0)
    ...
    def test_no_sleep_no_wait(self):
        """Must return quickly — no actual delay."""
        t0 = time.time()
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "idle")
        elapsed = time.time() - t0
        self.assertEqual(rc, 0)
        self.assertLess(elapsed, 2.0, "simulate-step should not sleep/wait")


class TestSimulateStepBlocked(unittest.TestCase):
    def setUp(self):
        self.root = Path(tempfile.mkdtemp())
        _setup_ready(self.root)

    def test_payment_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "payment")
        self.assertEqual(rc, 1)
        self.assertIn("simulation_status: blocked", out)
        self.assertIn("session_action: stop", out)

    def test_transaction_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "transaction")
        self.assertEqual(rc, 1)

    def test_service_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "service")
        self.assertEqual(rc, 1)

    def test_error_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "error")
        self.assertEqual(rc, 1)

    def test_maintenance_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "maintenance")
        self.assertEqual(rc, 1)

    def test_offline_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "offline")
        self.assertEqual(rc, 1)

    def test_unknown_blocked(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "unknown")
        self.assertEqual(rc, 1)


class TestSimulateStepNotReady(unittest.TestCase):
    def setUp(self):
        self.root = Path(tempfile.mkdtemp())
    ...
    def test_not_ready(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "idle")
        self.assertEqual(rc, 1)
        # When playlist is not ready, safety returns allowed=false → simulation=blocked
        self.assertIn("simulation_status: blocked", out)


class TestSimulateStepArgs(unittest.TestCase):
    def test_missing_root(self):
        out, err, rc = _run_cli("simulate-step", "--state", "idle")
        self.assertEqual(rc, 2)

    def test_missing_state(self):
        out, err, rc = _run_cli("simulate-step", "--root", "/tmp/xyz")
        self.assertEqual(rc, 2)

    def test_invalid_state(self):
        out, err, rc = _run_cli("simulate-step", "--root", "/tmp/xyz", "--state", "watching_tv")
        self.assertEqual(rc, 2)


class TestSimulateStepSecurity(unittest.TestCase):
    def setUp(self):
        self.root = Path(tempfile.mkdtemp())
        _setup_ready(self.root)
    ...
    def test_no_forbidden(self):
        for state in ["idle", "payment"]:
            out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", state)
            combined = (out + err).lower()
            for fb in FORBIDDEN:
                self.assertNotIn(fb, combined,
                                 f"simulate-step state='{state}' contains forbidden '{fb}'")
    ...
    def test_no_media_bytes(self):
        ...
        self.assertNotIn("\x89", out + err)

    def test_no_stacktrace(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "idle")
        self.assertNotIn("Traceback", out + err)
        self.assertNotIn('File "', out + err)


class TestSimulateStepNoIO(unittest.TestCase):
    def setUp(self):
        self.root = Path(tempfile.mkdtemp())
        _setup_ready(self.root)

    def test_no_backend_no_poP(self):
        out, err, rc = _run_cli("simulate-step", "--root", str(self.root), "--state", "idle")
        self.assertEqual(rc, 0)
    ...
    def test_no_secret_config_token(self):
        ...
        out, err, rc = _run_cli("simulate-step", "--root", str(root2), "--state", "payment")
        self.assertEqual(rc, 1)
