...
- ✅ Manifest sync (`/manifest/current`, `/manifest/{id}`) — внутренний клиент, пока только в памяти
- ❌ Manifest запись на диск / локальная раскладка
...
## ManifestClient (внутренний модуль)

Внутренний Python-модуль `manifest_client.py` — клиент для получения manifest:

- **`ManifestClient`** — выполняет `GET /api/device-gateway/manifest/current` и `GET /api/device-gateway/manifest/{id}`
- **`ManifestSnapshot`** — safe результат с метаданными (без полного manifest в summary)
- **Manifest пока только в памяти** — запись на диск не реализована
- **Retry пока не подключён** — будет отдельным шагом
- **Media download не реализован**
- **Валидация:** forbidden keys/values, path traversal в filename, UUID, sha256, duration, order
