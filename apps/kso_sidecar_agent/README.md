...
## KSO Gateway Manifest Extraction

**Реализован:** `kso_manifest_gateway_extractor.py`.

Sidecar получает backend gateway response:
```json
{
  "status": "served",
  "manifest_version_id": "...",
...
**Extractor** (`extract_kso_safe_manifest_body_from_gateway_response`):
- `status=served` → извлекает и валидирует `response["manifest"]`
- `status=not_modified` → не пишет, возвращает `not_modified`
- `status=no_manifest` → не пишет, возвращает `no_manifest`
- Invalid wrapper → `invalid_gateway_response`

**Writer** (`write_kso_safe_local_manifest_from_gateway_response`):
- Пишет **только** `response["manifest"]` (safe manifest body)
- Файл: `manifest/current_manifest.json`
- Атомарно: tmp → replace
- НЕ пишет wrapper-поля (manifest_version_id, manifest_hash, status, published_at)

**Локальный manifest содержит только**:
```json
{
  "schemaVersion": 1,
  "generatedAt": "ISO8601",
  ...
  "items": [{ "slotOrder": 0, "contentType": "image/png", "mediaRef": "media/current/slot-000", ... }]
}
```

**Запрещено в локальном manifest'е**: manifest_version_id, manifest_hash, status,
published_at, token, secret, campaign_id, creative_id, filename, sha256, ...
(полный список в `FORBIDDEN_MANIFEST_KEYS`).
...
## PoP Fake End-to-End Pipeline

🧪 **Реализован:** `tests/test_pop_e2e_fake_pipeline.py`. Сквозные тесты полной PoP-цепочки на fake backend.

### Покрытые сценарии

| # | Сценарий | Ожидание |
|---|---|---|
| 1 | Completed eligible + fake 200 | sent_records=1, sent/ создан, pending пуст |
| 2 | Fake 500 × 3 retry exhausted | rotation=false, pending untouched, sent/ не создан |
| 3 | Fake 409 duplicate | rotation=false, reason=duplicate_pending_remains |
| 4 | Draft event | send=false, rotation=false, HTTP не вызывается |
| 5 | Invalid JSON | send=false, rotation=false |
| 6 | Fingerprint mismatch (race) | sent=0, changed line retained |

### Что проверено

- ✅ Player-like event → `pop/pending/player_events.jsonl`
- ✅ `run_pop_scoped_send_then_rotate()` — полный цикл
- ✅ FakeHttpClient (no real backend)
- ✅ No secret/config/token/media bytes reads
- ✅ Safe output/repr без forbidden substrings во всех сценариях

### Следующий блок

После этого шага — **Блок 27: Linux KSO Player Runtime** (Chromium kiosk-mode, systemd, production event loop).
